...
const lobbyStartBtn  = document.getElementById("start-button");
...
function renderLobbyOverlay() {
  ...
  const canStart = !isActive && waiting.length > 0;
  ...
  const statusParts = [];
  statusParts.push(`Spies ready: ${waiting.length}/${lobbyState.maxPlayers}`);
  if (!isActive) {
    statusParts.push("Press start when ready");
  }
  if (isActive) {
  ...
  if (lobbyStartBtn) {
    ...
    lobbyStartBtn.textContent = isActive ? "Match Running" : "Start Mission";
    lobbyStartBtn.classList.toggle("disabled", lobbyStartBtn.disabled);
  }
}
